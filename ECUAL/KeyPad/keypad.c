    ...
    for(col=0;col<N_COL;col++)
    {
        ...
        case 0:
            ...
            for(row=0;row<N_ROW;row++)
            {
                ...
                {
                case 0:
                    if(GET_BIT(GPIO_PORTB_DATA_R,GPIO_PIN2) == 0)
                    {
                        return  KeyPad_3x3_switch(row,col);

                    }
                    break;
                case 1:
                    if(GET_BIT(GPIO_PORTB_DATA_R,GPIO_PIN3) == 0)
                    {
                        return  KeyPad_3x3_switch(row,col);
                    }
                    break;
                case 2:
                    if(GET_BIT(GPIO_PORTB_DATA_R,GPIO_PIN6) == 0)
                    {
                        return  KeyPad_3x3_switch(row,col);
                    }
                    break;
